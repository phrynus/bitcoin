...
func (t *TC) Start() (chan struct{}, chan error) {
	...
	bookTicker, err := t.acc.Client.NewListBookTickersService().Symbol(t.Symbol + "USDC").Do(context.Background())
	if err != nil {
		t.acc.Log.Errorf("获取 %sUSDC 盘口失败: %v", t.Symbol, err)
	}
	t.acc.Log.Debugf("组合单 %s: 获取到 %d 条盘口数据", t.ID, len(bookTicker))
	if len(bookTicker) == 0 {
	...
	err = RetryFunc(GetEnv().RetryCount, func() error {
		_, err := t.acc.Client.NewCreateOrderService().
...
func (t *TC) cancelOrder() error {
	...
	return RetryFunc(GetEnv().RetryCount, func() error {
		_, err := t.acc.Client.NewCancelOrderService().
			Symbol(t.Symbol + "USDC").
...
func (a *Account) CreateTC(symbol string, usdt, q decimal.Decimal) {
	...
	bookTicker, err := a.Client.NewListBookTickersService().Symbol(symbol + "USDC").Do(context.Background())
	if err != nil {
	...
	if err != nil {
		a.Log.Errorf("平掉 USDC 空仓失败: %v", err)
	}
