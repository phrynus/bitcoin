...
// OrderCancel 撤销订单(order.cancel, 签名接口)。
// symbol 必填; orderId 与 origClientOrderId 至少传一个(两者同时传时以 orderId 为准)。
// 返回撤销后的订单信息(字段结构与 WsOrder 一致)。
func (w *WsApi) OrderCancel(ctx context.Context, symbol string, orderID int64, origClientOrderID string) (*WsOrder, error) {
	if symbol == "" {
		return nil, errors.New("撤单缺少 symbol")
	}
	p := map[string]any{"symbol": symbol}
	switch {
	case orderID > 0:
		p["orderId"] = orderID
	...
	default:
		return nil, errors.New("撤单必须指定 orderId 或 origClientOrderId")
	}
	return Call[*WsOrder](w, ctx, "order.cancel", p, true)
}
