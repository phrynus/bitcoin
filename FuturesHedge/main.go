...
func init() {
	...
	if err != nil {
		log.Fatalf("[初始化] 获取交易所信息失败: %v", err)
	}

	log.Println("[初始化] 启动用户数据处理器")
	Uh = NewFuturesUserHandler(Client)
	uhComplete, err := Uh.Start()
	if err != nil {
		log.Fatalf("[初始化] 启动用户数据处理器失败: %v", err)
	}
	log.Println("[初始化] 等待用户数据流预热完成")
	<-uhComplete
	log.Println("[初始化] 用户数据处理器已就绪")
}
...
func main() {
	...
	go func() {
		for {
			log.Println("[主循环] ===== 开始新一轮处理 =====")
			GetTCPositions()

			if MarginRate == nil {
				log.Println("[主循环] 保证金率不可用，休眠 10 分钟")
				time.Sleep(Env.MainLoopIntervalDuration)
				continue
			}

			log.Printf("[主循环] 当前保证金率=%s%%", formatDecimalFixed(MarginRate.MarginRatio, 4))

			if !MarginRate.MarginRatio.IsZero() {
				log.Println("[主循环] 开始检查双边仓位是否平衡")
				if BalancePositions() {
					log.Println("[主循环] 已提交再平衡订单，跳过本轮剩余处理")
					continue
				}
			}
			...
			if MarginRate.MarginRatio.GreaterThan(Env.MarginRatioReduceTrigger) {
				...
				log.Println("[主循环] 保证金率处于安全区间")
			}

			log.Printf("[主循环] 本轮处理完成，休眠 %s", Env.MainLoopIntervalDuration.String())
			time.Sleep(Env.MainLoopIntervalDuration)

			if err := RefreshEnv(); err != nil {
				log.Printf("[主循环] 刷新配置失败: %v", err)
			}
		}
	}()
...
func GetTCPositions() {
	...
	if err != nil {
		log.Printf("[仓位刷新] 计算保证金率失败: %v", err)
		return
	}
...
func HasSufficientAvailableBalance(scene string) bool {
	if MarginRate == nil {
		log.Printf("[%s] 保证金率结果为空，跳过下单", scene)
		return false
	}

	if MarginRate.TotalAvailable.LessThan(Env.MinAvailableUSD) {
		log.Printf("[%s] 当前总可用资金=%s USD，低于阈值=%s USD，停止继续下单",
			scene,
...
func MarginRatioBeyond() {
	...
	log.Println("[高保证金率处理] 开始执行")
	for {
		symbol := GetMaxProfitSymbol()
		if symbol == "" {
			log.Println("[高保证金率处理] 未找到可减仓的盈利币种")
			return
		}

		log.Printf("[高保证金率处理] 当前保证金率=%s%%", formatDecimalFixed(MarginRate.MarginRatio, 4))
		if MarginRate.MarginRatio.LessThan(Env.MarginRatioReduceTarget) {
			log.Println("[高保证金率处理] 保证金率已降到 45%% 以下，结束减仓")
			break
		}
...
func MarginRatioSmall() {
	...
	for i := 0; i < 10; i++ {
		...
		log.Printf("[低保证金率处理] 当前保证金率=%s%%", formatDecimalFixed(MarginRate.MarginRatio, 4))
		...
		if symbolConfig == nil {
			log.Printf("[低保证金率处理] 未找到 %s 的配置", symbol)
			return
		}
		...
		case <-doneC:
			...
			GetTCPositions()
		case err := <-errCh:
			log.Printf("[低保证金率处理] 套餐单执行失败: %v", err)
